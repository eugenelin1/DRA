...
class Test_DRA(object):
    def __init__(self, sess, epoch = 200, lr=0.0001, beta1=0.5, batch_size=128, X_dim=720, z_dim=10, dataset_name='mnist',
                 ...
                 g_iter = 2, lam=10.0, control = 3, sampler='uniform'):
    ...
    def train_cluster(self):
        ...
        control = 3 # Generator is updated twice for each discriminator update
    ...
    @property
    def model_dir(self):
        s = "DRA_{}_{}_b_{}_g{}_d{}_{}_{}_lr_{}_b1_{}_leak_{}_keep_{}_z_{}_{}_bn_{}_lam_{}_giter_{}_epoch_{}_ctrl_{}".format(
            ...
            self.lam, self.g_iter, self.epoch, self.control) 
        s = s.replace('[', '_')
        s = s.replace(']', '_')
        s = s.replace(' ', '')
        return s
...
if __name__=='__main__':
    ...
    flags.DEFINE_integer("control", 3, "control") 
    FLAGS = flags.FLAGS
    ...
    print ("control: {}".format(FLAGS.control))     
